...
export function createArcade(config) {
  ...
  // sessionStorage slot so a reload keeps the player's identity (and board row)
  // within the tab. The key is a throwaway testnet key with no funds.
  const SK_KEY = "degendash.ephemeral";
  ...
  async function loadLibs() {
    ...
    try {
      ...
      console.warn("[degendash] crypto libs failed to load:", err);
  ...
  /** Load libs, restore/generate the ephemeral key, fetch the starting nonce.
  ...
  // collect fires a real grab at one streaming item. The CONTRACT decides, by the
  // real block timestamp, whether it landed inside the catch window — an in-window
  // grab of a GOOD item scores, of a BAD item penalizes. Never client-reported, so
  // the score is uncheatable.
  function collect(roundId, itemId) {
    return send(
      "collect",
      `collect@${intToArgHex(roundId)}@${intToArgHex(itemId)}`,
      cfg.gas.collect,
    );
  }
  // endRound tallies the final score once the round is fully over.
  function endRound(roundId) {
    return send("endRound", `endRound@${intToArgHex(roundId)}`, cfg.gas.endRound);
  }
  // claim mints your collected NOVA to your wallet (real ESDT). No-op if no token set.
  function claim() {
  ...
  async function getLeaderboard(n = 10) {
    try {
      ...
      console.warn("[degendash] getLeaderboard failed:", err);
      return [];
    }
  }
  ...
  async function getGlobalActions() {
    try {
      ...
      console.warn("[degendash] getGlobalActions failed:", err);
      return null;
    }
  }
   ...
   *  { roundId, seed, startMs, totalGood, ended }. `seed` is a decimal STRING
  ...
  async function getRound(address) {
    try {
      ...
      return {
        ...
        totalGood: decodeU64(parts[3]),
        ended: !!(parts[4] && decodeU64(parts[4]) !== 0),
      };
    } catch (err) {
      console.warn("[degendash] getRound failed:", err);
      return null;
    }
  }

  /** The contract-computed final score for the player's current round. */
  async function getFinal(address) {
    try {
      ...
      console.warn("[degendash] getFinal failed:", err);
      return null;
    }
  }
  ...
  async function getClaimable(address) {
    try {
      ...
      console.warn("[degendash] getClaimable failed:", err);
      return null;
    }
  }

  /** The shared difficulty config (all times in MILLISECONDS):
   *  { roundLengthMs, catchWindowMs, items, goodPermil, rampPermil,
   *    lateWindowShrinkPermil }. */
  async function getScheduleParams() {
    try {
      ...
      return {
        ...
        catchWindowMs: decodeU64(parts[1]),
        items: decodeU64(parts[2]),
        goodPermil: decodeU64(parts[3]),
        rampPermil: parts[4] !== undefined ? decodeU64(parts[4]) : 0,
        lateWindowShrinkPermil: parts[5] !== undefined ? decodeU64(parts[5]) : 0,
      };
    } catch (err) {
      console.warn("[degendash] getScheduleParams failed:", err);
      return null;
    }
  }
  ...
  async function getPlayerEntry(address) {
    ...
    try {
      ...
      console.warn("[degendash] getPlayerEntry failed:", err);
